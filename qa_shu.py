...
def clean_texts(session: int):
    ...
    remove_text_from_md_files(base_dir, """経過へ\n|\n質問本文(PDF)へ""")
    base_dir = f"data/qa_shu/complete/{session}/q"
    delete_md_files_with_message(
        base_dir,
        "ＨＴＭＬファイルについてはしばらくお待ちください。ＰＤＦファイルをご覧ください。",
    )
...
save_qa_shu_question_texts(LATEST_SESSION, wait_second=WAIT_SECOND)
save_qa_shu_answer_texts(LATEST_SESSION, wait_second=WAIT_SECOND)
clean_texts(LATEST_SESSION)

# ステータス保存（質問ごと）
for q in data.questions:
    save_status_if_needed(
        LATEST_SESSION, q, wait_second=WAIT_SECOND, force_if_not_received=True
    )
