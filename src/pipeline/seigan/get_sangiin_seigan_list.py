"""参議院サイトの請願一覧ページを取得して raw HTML を保存する。
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="指定した回次の参議院請願一覧 HTML を取得する")
...
def process_session(session: int, output_dir: Path = OUTPUT_DIR, skip_existing: bool = False) -> Path:
    ...
    logger.info("参議院請願一覧HTML取得開始: session=%s url=%s", session, source_url)
    ...
    logger.info("参議院請願一覧HTML取得完了: session=%s", session)
    return output_path
